            ...
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center gap-8">
                    <router-link to="/" class="flex items-center gap-2">
                        ...
                        <span
                            class="text-xl font-bold bg-gradient-to-r from-emerald-600 to-teal-700 bg-clip-text text-transparent">
                            BookACourt
                        </span>
                    </router-link>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <router-link v-if="authStore.isAuthenticated" to="/notifications"
                        class="relative p-2 text-gray-600 hover:bg-emerald-50 rounded-lg transition-colors">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                        </svg>
                        <span v-if="unreadCount > 0"
                            class="absolute top-1 right-1 w-5 h-5 bg-emerald-600 text-white text-xs rounded-full flex items-center justify-center font-bold">
                    ...
                    <div v-if="authStore.isAuthenticated" class="relative">
                        <button @click.stop="toggleUserMenu"
                            class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-emerald-50 transition-colors">
                            <div
                                class="w-8 h-8 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                        ...
                        <transition name="dropdown">
                            <div v-if="showUserMenu"
                                class="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-200 py-1">
                                <div class="px-4 py-3 border-b border-gray-200">
                                    <p class="text-sm font-medium text-gray-900">{{ authStore.user?.full_name || 'User'
                                    }}</p>
                                    <p class="text-xs text-gray-500 mt-1">{{ authStore.user?.phone_number || 'No phone'
                                    }}</p>
                                </div>
                    ...
                    <router-link v-else to="/login"
                        class="px-5 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-lg hover:shadow-lg hover:brightness-110 transition-all font-medium">
                        Login
                    </router-link>

                    <!-- Mobile Menu Button -->
                    <button @click="showMobileMenu = !showMobileMenu"
                        class="md:hidden p-2 text-gray-600 hover:bg-emerald-50 rounded-lg transition-colors">
